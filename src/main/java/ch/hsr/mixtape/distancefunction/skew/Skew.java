...
public class Skew {
	...
	/*
	 * if length mod 3 == 2 -> sorting goes berserk ! reason : length mod1 1
	 * bigger than length mod 2
	 */
	...
	public int[] buildSuffixArray(double[] values) {
		...
		int lengthMod0 = values.length % 3 != 0 ? values.length / 3 + 1
				: values.length / 3;
		int lengthMod1 = values.length % 3 == 2 ? values.length / 3 + 1
				: values.length / 3;
		...
		int[] indicesMod12 = findMod12Positions(values.length + lengthDifMod12,
				lengthMod12 + lengthDifMod12);
		...
		suffixArrayMod0 = sort(indicesMod0, input, 0);
