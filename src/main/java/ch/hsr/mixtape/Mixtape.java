...
public class Mixtape {
	...
	private static double[][][] calcDistances(List<Song> songs,
		...
		for (int x = 0; x < songs.size(); x++) {
			...
			System.out.println("1. before gc "
					+ Runtime.getRuntime().freeMemory());
			Runtime.getRuntime().gc();
			System.out.println("1. after gc "
					+ Runtime.getRuntime().freeMemory());
			...
			for (int y = 0; y < x; y++) {
				...
				System.out.println("distances between " + nameX + " and "
						+ nameY + " = " + Arrays.toString(distances[x][y]));
			}

			System.out.println("2. before gc "
					+ Runtime.getRuntime().freeMemory());
			Runtime.getRuntime().gc();
	...
	public static void main(String[] args) throws InterruptedException,
		...
		for (int x = 0; x < songs.size(); x++) {
			...
			for (int y = 0; y < x; y++) {
				...
				System.out.println(new File(songX.getFilePath()).getName()
						+ " to "
						+ new File(songY.getFilePath()).getName()
						+ " = "
						+ mixtape.distanceBetween(x, y, new double[] { 1, 1, 1,
								1 }));
			}
		}
	}
