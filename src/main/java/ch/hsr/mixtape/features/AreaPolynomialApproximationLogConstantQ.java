...
public class AreaPolynomialApproximationLogConstantQ {

	private static final int WINDOW_LENGTH = 50;
	private static final int FEATURE_LENGTH = 20;
	...
	public double[] extractFeature(double[] samples, double sampling_rate, double[][] logOfConstantQs) {
		if ((FEATURE_LENGTH != logOfConstantQs[0].length) || (WINDOW_LENGTH != logOfConstantQs.length)) {
			terms = calcTerms();
			;
			z = new DenseDoubleMatrix2D(1, FEATURE_LENGTH * WINDOW_LENGTH);
		}
		for (int i = 0; i < WINDOW_LENGTH; ++i) {
			for (int j = 0; j < FEATURE_LENGTH; ++j) {
				z.set(0, WINDOW_LENGTH * i + j, logOfConstantQs[i][j]);
			}
		}
