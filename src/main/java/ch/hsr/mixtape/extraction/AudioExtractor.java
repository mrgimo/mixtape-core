...
public class AudioExtractor implements Runnable {
	...
	private void extractFeatures(double[] samples) {
		...
		ArrayList<SpectralFeature> extractedFeatures = featureController.extractFeatures(samples);

		generateFeatureVector(extractedFeatures);
	}

	private void generateFeatureVector(ArrayList<SpectralFeature> extractedFeatures) {
